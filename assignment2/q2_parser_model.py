...
class ParserModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0.):
        ...
        ### YOUR CODE HERE
        feed_dict = {}
        # print("input: {}".format(inputs_batch[:10, :10]))
        # print("dropout p: {}".format(dropout))
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        ev = tf.Variable(initial_value=self.pretrained_embeddings, name="pretrained_embeddings")
        el = tf.nn.embedding_lookup(ev, self.input_placeholder)
        embeddings = tf.reshape(el, (-1, self.config.n_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b = tf.get_variable("b", shape=self.config.hidden_size, initializer=tf.zeros_initializer)
        ...
        hdrop = tf.nn.dropout(h, 1. - self.dropout_placeholder)
        pred = tf.matmul(hdrop, U) + b2
        # ### END YOUR CODE
    ...
    def __init__(self, config: Config, pretrained_embeddings):
